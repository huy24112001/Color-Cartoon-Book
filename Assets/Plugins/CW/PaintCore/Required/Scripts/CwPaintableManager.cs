...
namespace PaintCore
{
	...
	public class CwPaintableManager : MonoBehaviour
	{
		...
		private static LinkedList<CwPaintableManager> instances = new LinkedList<CwPaintableManager>(); private LinkedListNode<CwPaintableManager> instancesNode;
		...
		public int ReadPixelsBudget { set { readPixelsBudget = value; } get { return readPixelsBudget; } }
		[SerializeField] private int readPixelsBudget = 4096;
		...
		public static void SubmitAll(CwCommand command, Vector3 position, float radius, int layerMask, CwGroup group, CwModel targetModel, CwPaintableTexture targetTexture)
		{
			...
			for (var c = 0; c < CwClone.ClonerCount; c++)
			{
				for (var m = 0; m < CwClone.MatrixCount; m++)
				{
					...
					DoSubmitAll(copy, position, radius * (1 + m / 10), layerMask, group, targetModel, targetTexture);
